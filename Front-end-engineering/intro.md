...
思考一下这样的场景：按订单状态查看订单表列，设订单状态包含`['待支付', '已支付', '已取消']`。
...
事件上这也是服务端渲染模版的方式：拿到数据 -> 传入模版引擎 -> 得到结果字符 -> 传递到客户端

# 与用户行为交互

与服务端不同，客户端最重要的功能除了内容展现之外，还要提供用户交互界面(UI)。
常见的交互造成的结果包括`View`和`Model`更新。
